...
public:
    ...
    tsl::elm::Element* createUI() override {
        ...
        list->addItem(new tsl::elm::CategoryHeader("国行卡带(NS) - 010000000000001F"));
        list->addItem(config_ns1.create_list_item("解锁卡带区域限制"));
    ...
    tsl::elm::Element* createUI() override {
        ...
        if (does_file_exist(LOG_PATH)) {
            ...
            ini_browse([](const mTCHAR *Section, const mTCHAR *Key, const mTCHAR *Value, void *UserData){
                ...
                } else if (status.starts_with("未修补") || status.starts_with("已禁用")) {
